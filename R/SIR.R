...
run.SIR <- function(nrep,
  ...
  if (is.null(pen$carry.dist)) {
    ...
    pen$carry.min <- Carry / Kscale
    pen$carry.max <- Carry * Kscale
  ...
  get.keepers <- function(Nkeep, CumLike, BreakPoints) {
    ...
    for (i in 1:Nkeep) {
      Keepers[i] <-
        which(CumLike > BreakPoints[i])[1] - 1 # since that zero was tacked on
    }
    return(sort(Keepers))
  }
